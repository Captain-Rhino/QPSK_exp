...
M = 4; 
%genqammod  = modem.pskmod(M,pi/4) ;
%H_demod = modem.pskdemod(M,pi/4) ;

codeRate = 1/2; constlen = 7; k = log2(M); codegen = [171 133];
tblen = 32;     % traceback length
trellis = poly2trellis(constlen, codegen);%生成卷积码编码译码所需之网格图
...
EbNoDemo = 3; EsN0 = EbNoDemo + 10*log10(k);     %由于dB导致的Es/N0 = Eb*log2(M)/N0                  
seed = [654321 123456];              
rand('state', seed(1));  randn('state', seed(2));              
msg_orig = randsrc(numSymb, 1, 0:1);   %生成随机的0，1                   
...
[msg_enc_bi] = convenc(msg_orig, trellis);%卷积编码
...
%msg_tx = H_mod( msg_gr_enc);
msg_tx = pskmod(msg_gr_enc,M,pi/4);%psk调制

msg_tx = rectpulse(msg_tx, Nsamp);%方波图
msg_rx = awgn(msg_tx, EsN0-10*log10(1/codeRate)-10*log10(Nsamp));%高斯白噪声信道
...
%msg_gr_demod = H_demod( msg_rx_int);
msg_gr_demod = pskdemod(msg_rx_int,M,pi/4);%qpsk解调
[dummy graydecod] = sort(grayencod); graydecod = graydecod - 1;
msg_demod = graydecod(msg_gr_demod+1)';%格雷码_to_binary
...
msg_dec = vitdec(msg_demod_bi, trellis, tblen, 'cont', 'hard');%viterbi解码
...
[nChnlErrs BERChnl] = biterr(msg_enc, msg_demod);%卷积编码收发误码率
[nCodErrs BERCoded] = biterr(msg_orig(1:end-tblen), msg_dec(1+tblen:end));%接收码与源码误码率
